...
module.exports = {
    to_json:    async function(FOLDER,FILE){
        try {
                ...
                if(object[MONTH][state].REGION == REGION){
                    for (const DAY in object[MONTH][state]) {
                        if (object[MONTH][state][DAY].TEMP != undefined) {
                            ...
                            if(RAWDATA.TEMP.LOW > object[MONTH][state][DAY].TEMP.LOW)   RAWDATA.TEMP.LOW = object[MONTH][state][DAY].TEMP.LOW*1;
                            if(RAWDATA.TEMP.HIGH < object[MONTH][state][DAY].TEMP.HIGH) RAWDATA.TEMP.HIGH = object[MONTH][state][DAY].TEMP.HIGH*1;
                            ...
                            if(RAWDATA.WIND.MAX < object[MONTH][state][DAY].WIND.MAX)   RAWDATA.WIND.MAX = object[MONTH][state][DAY].WIND.MAX*1;
                                            
                            RAWDATA.HUMI.AVR += object[MONTH][state][DAY].HUMI.AVR*1;
                            RAWDATA.HUMI.DEW += object[MONTH][state][DAY].HUMI.DEW*1;
                            if(RAWDATA.HUMI.MIN > object[MONTH][state][DAY].HUMI.MIN)   RAWDATA.HUMI.MIN = object[MONTH][state][DAY].HUMI.MIN*1;
                            
                            RAWDATA.SUN.SUM += object[MONTH][state][DAY].SUN.SUM*1;
                            if(RAWDATA.SUN.PH < object[MONTH][state][DAY].SUN.PH)       RAWDATA.SUN.PH = object[MONTH][state][DAY].SUN.PH*1;
                        }
                    }
                }       
            }
            const DATA = {
                TEMP: { AVR: (RAWDATA.TEMP.AVR/RAWDATA.SAMPLE).toFixed(2)*1, LOW: (RAWDATA.TEMP.LOW).toFixed(2)*1, HIGH: (RAWDATA.TEMP.HIGH).toFixed(2)*1 },
                RAIN: (RAWDATA.RAIN/RAWDATA.SAMPLE).toFixed(2)*1,
                WIND: { AVR: (RAWDATA.WIND.AVR/RAWDATA.SAMPLE).toFixed(2)*1, MAX: (RAWDATA.WIND.MAX).toFixed(2)*1, DIR: (RAWDATA.WIND.DIR/RAWDATA.SAMPLE).toFixed(2)*1},
                HUMI: { AVR: (RAWDATA.HUMI.AVR/RAWDATA.SAMPLE).toFixed(2)*1, MIN: (RAWDATA.HUMI.MIN).toFixed(2)*1, DEW: (RAWDATA.HUMI.DEW/RAWDATA.SAMPLE).toFixed(2)*1 },
